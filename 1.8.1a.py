...
count = 0 #глобальные переменные, которые отвечают за историю ввода
story_input = []
    ...
    while True:
        try:
            ...
            if switch_variable > 0 and switch_variable <= 4:
                ...
                print('Неверно введено число. Введите "1", "2" или "3" ')
        except ValueError:
            print("Неверно выполнен ввод. Введите число")
...
def get_input_limit():#ввод предела интегрирования
    ...
    while True:
        try:
            ...
            if (a <= b): return a, b
            else: print("Неверно введены пределы интегрирования. Первое число должно быть меньше второго")
...
def get_input_n():#ввод количества отрезков, на которые будет разделён предел интегрирования
    ...
    while True:
        try:
            n = int(input())
            if (n >= 1): return n
            else:  print("Невверно введено число (n >= 1)")
        except ValueError:
            print("Неверно выполнен ввод. Введите число")
...
def integral(): #подсчёт значения интеграла
    global count
    global story_input
...
def story(): #показывает историю ввода
    global count
    global story_input
...
def main():
    ...
    while True:
        ...
        elif switch_variable == 2: integral()
        elif switch_variable == 3: story()
        elif switch_variable == 4: return
